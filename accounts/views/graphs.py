...
def compare(request, *args, **kwargs):
    ...
    for tag in taglist:
        ...
        rows = qs.values('month').annotate(Sum('debit_amount'), Count('pk')).order_by('month')

        data[tag] = {
            'day': [r['month'] for r in rows],
            'count': [int(r['pk__count']) for r in rows],
            'amount': [float(r['debit_amount__sum']) for r in rows],
        }
